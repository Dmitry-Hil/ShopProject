...
export async function getOthers(id: string): Promise<IProduct[] | null> {
  try {
    const { data } = await axios.get<IProduct[]>(`${host}/products/others/${id}`);
    return data;
  } catch (e) {
    return null;
  }
...
export async function getSimilars(id: string): Promise<IProduct[] | null> {
  try {
    const { data } = await axios.get<IProduct[]>(`${host}/products/similars/${id}`);
    return data;
  } catch (e) {
    return null;
  }
...
export async function addProduct(
  ...
  try {
    let images: { url: string; main: boolean }[] | undefined;
    ...
    const response = await axios.post<{ productId: string }>(`${host}/products`, {
      title: formData.title,
      description: formData.description,
